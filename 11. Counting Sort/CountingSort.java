...
public class CountingSort
{
    ...
    private static final String LOG = "842986_countingsort.txt";
    private static final long MATRICULA = 842986;
    private static long tempoExecucao;

    // Maior taxa de captura possível (para o counting sort)
    private static final int MAX_CAPTURE_RATE = 255;
    ...
    private static void countingsort(List<Pokemon> vec)
    {
        int n = vec.size();
        List<Pokemon> output = new ArrayList<>(n);
        for (int i = 0; i < n; i++)
            output.add(null);

        int[] count = new int[MAX_CAPTURE_RATE + 1];

        // Inicializa o array de contagem.
        for (int i = 0; i <= MAX_CAPTURE_RATE; ++i)
            count[i] = 0;

        // Conta a ocorrência de cada taxa de captura.
        for (Pokemon p : vec)
            ++count[p.getCaptureRate()];

        // Modifica count[i] para que ele contenha a posição de saída
        // de cada taxa de captura.
        for (int i = 1; i <= MAX_CAPTURE_RATE; ++i)
            count[i] += count[i - 1];

        // Constrói o array de saída.
        for (int i = n - 1; i >= 0; i--) {
            Pokemon p = vec.get(i);
            output.set(count[p.getCaptureRate()] - 1, p);
            --count[p.getCaptureRate()];
        }

        // Copia o array de saída para vec, para que vec agora contenha
        // os elementos ordenados por taxa de captura.
        for (int i = 0; i < n; ++i)
            vec.set(i, output.get(i));

        // Aplica uma ordenação estável (insertion sort) nos nomes para
        // Pokémon com a mesma taxa de captura.
        desempataInsercao(vec);
    }

    private static void desempataInsercao(List<Pokemon> vec)
    {
        for (int i = 1; i < vec.size(); ++i) {
            Pokemon tmp = vec.get(i);
            int j;

            for (j = i - 1;
                 j >= 0 && vec.get(j).getCaptureRate() == tmp.getCaptureRate() &&
                 vec.get(j).getName().compareTo(tmp.getName()) > 0;
                 --j)
...
class Pokemon implements Comparable<Pokemon>, Cloneable
{
    ...
    // Ordena Pokémon por taxa de captura.
    @Override public int compareTo(Pokemon outro)
    {
        ++Pokemon.numComparacoes;
        int res = this.captureRate - outro.captureRate;
