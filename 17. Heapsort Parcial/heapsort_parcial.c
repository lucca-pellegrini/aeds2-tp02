...
#define LOG "842986_heapsortParcial.txt"
...
static void heapsort_parcial(Pokemon **vec, size_t n, size_t k);
static inline void construir_heap(Pokemon **vec, size_t n);
static void reconstruir_heap(Pokemon **vec, size_t n, size_t i);
...
// Compara um Pokémon com outro a partir da altura.
int compara(Pokemon *restrict const p1, Pokemon *restrict const p2)
{
	int res = (p1->height > p2->height) ? 1 :
...
int main(int argc, char **argv)
{
	...
	heapsort_parcial(selecionados, num_select, K);
...
static void heapsort_parcial(Pokemon **vec, size_t n, size_t k)
{
	...
	// Parte 1: construir o heap para os primeiros k elementos.
	construir_heap(vec, k);

	// Parte 2: para cada elemento de k até n - 1, faça a inserção parcial.
	for (size_t i = k; i < n; ++i) {
		if (compara(vec[i], vec[0]) > 0) {
			swap(vec, 0, i);
			reconstruir_heap(vec, k, 0);
		}
	}

	// Parte 3: ordenar os primeiros k elementos usando heapsort.
	for (size_t i = k - 1; i > 0; --i) {
		Pokemon *temp = vec[0];
		vec[0] = vec[i];
		vec[i] = temp;

		reconstruir_heap(vec, i, 0);
	}
}
...
static void reconstruir_heap(Pokemon **vec, size_t n, size_t i)
{
	...
	if (esq < n && compara(vec[esq], vec[raiz]) > 0)
		raiz = esq;

	if (dir < n && compara(vec[dir], vec[raiz]) > 0)
		raiz = dir;
