...
static void shellsort(Pokemon **vec, size_t n);
...
// Compara um Pokémon com outro a partir do peso.
int compara(Pokemon *restrict const p1, Pokemon *restrict const p2)
{
	int res = 0;
...
int main(int argc, char **argv)
{
	...
	shellsort(selecionados, num_select);
...
static void shellsort(Pokemon **vec, size_t n)
{
	for (size_t gap = n / 2; gap > 0; gap >>= 1) {
		for (size_t i = gap; i < n; ++i) {
			Pokemon *temp = vec[i];
			size_t j;

			for (j = i; j >= gap && compara(temp, vec[j - gap]) < 0;
			     j -= gap)
				vec[j] = vec[j - gap];
			vec[j] = temp;
		}
