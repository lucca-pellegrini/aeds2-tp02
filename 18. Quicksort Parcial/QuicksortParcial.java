...
public class QuicksortParcial
{
    ...
    private static final String LOG = "842986_quicksortParcial.txt";
    ...
    public static void main(String[] args)
    {
        ...
        try (Scanner sc = new Scanner(System.in)) {
            ...
            quicksortParcial(selecionados, K);
    ...
    public static <T extends Comparable<T> > void quicksortParcial(List<T> vec, int k)
    {
        if (vec != null && vec.size() > 0 && k > 0)
            quicksort(vec, 0, vec.size() - 1, k);
    }

    private static <T extends Comparable<T> > void quicksort(List<T> vec, int esq,
                                                             int dir, int k)
    {
        int i = esq, j = dir;
        T pivo = vec.get((esq + dir) / 2);

        while (i <= j) {
            while (vec.get(i).compareTo(pivo) < 0)
                ++i;
            while (vec.get(j).compareTo(pivo) > 0)
                --j;
            if (i <= j)
                swap(vec, i++, j--);
        }
        ...
        if (esq < j)
            quicksort(vec, esq, j, k);
        if (i < k && i < dir)
            quicksort(vec, i, dir, k);
    }
