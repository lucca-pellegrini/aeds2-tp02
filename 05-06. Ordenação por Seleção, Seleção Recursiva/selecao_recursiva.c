...
static void selecao_recursiva(Pokemon **vec, size_t n);
...
// Compara um Pokémon com outro a partir do nome.
int compara(Pokemon *restrict const p1, Pokemon *restrict const p2)
{
...
#define LOG "matrícula_selecaoRecursiva.txt"
...
int main(int argc, char **argv)
{
	...
	selecao_recursiva(selecionados, num_select);
...
static void selecao_recursiva(Pokemon **vec, size_t n)
{
	if (n) {
		...
		for (size_t i = 1; i < n; ++i) {
			++num_comparacoes;
			if (compara(vec[i], vec[menor]) < 0)
				menor = i;
		}

		swap(vec, 0, menor);
		selecao_recursiva(vec + 1, n - 1);
	}
}
